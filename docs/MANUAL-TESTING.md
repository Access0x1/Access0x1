...
the automated suite proves (846 contract tests + the web gate) you will drive by
...
- [D. The test suites (846 + web gate)](#d-the-test-suites-846--web-gate)
...
### A5. Deploy the whole suite to Anvil — the keyless path
...
`Suite result: ok. 846 passed; 0 failed; 0 skipped`. This is the union of:
...
**Expected:** contracts build + 846 tests + `forge fmt --check`, then the web
...
- [ ] **Deps installed.** `make install` completed; a bare `make build` exits
      green (Section A3).
- [ ] **Contracts green.** `make test` → `846 passed; 0 failed` (Section D1).
- [ ] **Web green.** `cd web && npm run gate` → all four steps pass, ~532 tests
      (Section D2).
