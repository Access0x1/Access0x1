...
| Symbolic execution | `make halmos` | ⚠️ **2 of 4 proofs do not actually run — found 2026-08-28, real gap, not new.** `FeeSplitSymbolic`'s 2 proofs pass (fixed a spurious timeout by raising the solver budget in the Makefile — the property was never violated, just under-timed). `SessionBudgetSymbolic`'s 2 proofs (`check_spend_neverExceedsBudget`, `check_twoSpends_neverExceedBudget`) **ERROR** on `Unsupported cheat code: expectRevert()` — Halmos does not support that Foundry cheatcode; halmos's own docs say to use a low-level `.call` and check the boolean instead. **This means the SessionGrant budget-cap property has likely never been symbolically verified**, despite the historical audit record claiming it passes. See `FINDINGS.md` for the fix (not applied — flagged for sign-off, it changes how a money-safety property is verified). |
