...
| Whole-suite gate (current) | **1,392 tests passed, 0 failed, 0 skipped** across **104 suites** (unit + attack + invariant + integration + fuzz + scenario + fork + symbolic). Coverage is instrumented over the non-fork suites; the 3 `test/fork/**` tests are no-ops without a fork RPC and do not affect line coverage. |
...
inside the 1,392-test total and refresh into this table on the next run.
