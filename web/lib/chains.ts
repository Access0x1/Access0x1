 ...
 * endpoint tier. A booth run sharing one venue IP can trip that, so the RPC is
...
export function getRpcUrl(chainId: number): string {
  ...
  // configured, which under booth load is a rate-limit lottery whose loss shows up as a
