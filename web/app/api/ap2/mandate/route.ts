 ...
 *
 *   200  { ok: true, mandates }              full Intent (+ Cart + Payment if cart/payment supplied)
 *   200  { ok: true, mandates, linksValid }  when a full chain is built
 ...
 *    unsigned VC up as signed. The `note` in the response says the JWS is env-keyed at deploy.
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    ...
    // Intent only.
    if (!validated.cart) {
      return json({ ok: true, mandates: { intent }, note }, 200);
    }
    ...
    if (!validated.payment) {
      return json({ ok: true, mandates: { intent, cart }, note }, 200);
    }
    ...
    return json(
      { ok: true, mandates: chain, linksValid: verifyChainLinks(chain) === null, note },
      200,
    );
  } catch (err) {
